...
class _NotasPageState extends State<NotasPage> {
  ...
  final Stream<QuerySnapshot> _notasStream = FirebaseFirestore.instance
      .collection('notas')
      .orderBy('dataHora', descending: true)
      .snapshots(); // conecta ao firebase uma unica vez, quando abrir a tela,
  ...
  void initState() {
    ...
    _tituloController.addListener(() {
      // cria meio q um fiscal pra ficar vigiando o campo de texto do titulo da captura rapida
      if (mounted)
        setState(
          () {},
        ); // o mounted serve pra evitar crash, nesse caso seria tipo um "se a tela ainda existir(ou seja, se o usuario nao tiver trocado de aba), redesenhe ela"
    });
    _conteudoController.addListener(() {
      // mesma coisa, so que com o conteudo, redesenhando a tela a cada letra digitada pelo usuario
      if (mounted)
        setState(
          () {},
        ); // entao caso o usuario mudar de tela ou fechar a pagina antes de fechar o teclado, esse controller poderia tentar atualizar a tela ja morta
    }); // mas o mounted impede que isso aconteca.
    _buscaController.addListener(() {
      // mesmo processo, so que com o campo de busca, pra atualizar a lista de notas em tempo real conforme o usuario digita
      if (mounted)
        setState(
          () {},
        ); // entao a cada letra digitada na busca, a tela vai ser redesenhada e a lista de notas vai ser filtrada de novo, mostrando apenas as notas que batem com o texto da busca
    });
  }
  ...
  void _salvarNota() async {
    ...
    if (titulo.isEmpty || conteudo.isEmpty) {
      // isEmpty ja vem programado pelo flutter e dart. assim como isnotempty
      ScaffoldMessenger.of(context).showSnackBar(
        // olha pra tela atual, identifica onde pode escrever uma mensagem e mostra um aviso
    ...
    try {
      // a partir daqui, vamos testar algumas coisas que dependem de internet

      await _notasCollection.add(
        novaNota.toMap(),
      ); // esse await eh tipo um pause na execucao enquanto faz o processo de consulta ao firebase
      ...
      // se a internet cair no processo do await, ele pula direto pra ca e guarda o motivo do erro na variavel 'e'
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  void _deletarNota(String id) async {
    // esse async serve pra avisar ao dart
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        ...
        ), // avisa que deu errado, feedback do erro
      );
    }
  }
  ...
  Color _definirCorTextoSalvar() {
    if (_tituloController.text.trim().isNotEmpty &&
      ...
      return const Color.fromARGB(
        ...
        255,
        255,
      ); // branco meio opaco/cinza quando desativado
    }
  }
          ...
          actions: [
            ...
            TextButton(
              onPressed: () => Navigator.of(
                context,
              ).pop(), // fecha o alerta sem apagar a nota
              child: const Text(
                'Cancelar',
                style: TextStyle(
      ...
      body: SafeArea(
        child: ListView(
          ...
          children: [
            ...
            TextField(
              controller:
                  _buscaController, // aquele controle que le o que veio do textfield
            ...
            SingleChildScrollView(
              // melhor que o listview para listas pequenas, renderiza tudo de uma vez;
              scrollDirection: Axis
                  .horizontal, // scrolla pros lados p ver os filtros disponiveis
              child: Row(
                children: [
                  GestureDetector(
                    onTap: () => setState(() => _filtroCategoria = 'Todas'),
                    child: _construirFiltro(
                      'Todas',
            ...
            StreamBuilder<QuerySnapshot>(
              // eh um widget do flutter que atualiza a tela sozinho, redesenhando a lista sempre que algo muda no banco de dados
              stream:
                  _notasStream, // pega a colecao de notas do firebase, ordenando elas pela data, deixando as mais recentes em cima
              ...
              builder: (context, snapshot) {
                // ela eh quem desenha o resultado definitivo. o context localiza o bloco na tela e o snapshot guarda os dados que o firebase devolveu
                ...
                if (snapshot.hasError) {
                  ...
                  ); // se der erro de internet ou firebase, erro de feedback na tela
                }
                if (snapshot.connectionState == ConnectionState.waiting) {
                  return const Center(
                    child: CircularProgressIndicator(),
                  ); // se tiver carregando, pro usuario saber disso, mostra a bolinha girando na tela
                }

                final docs =
                    snapshot.data?.docs ??
                    []; // se o snapshot tiver dados, pega os documentos, se nao tiver, deixa como lista vazia, pra evitar crash

                final notasFiltradas = docs.map((doc) => Nota.fromDocument(doc)).where((
                  nota,
                ) {
                  // converte de volta o doc do firebase pro objeto nota
                  // esse .where eh quem faz a filtragem a partir dos True e False recebidos das condicoes abaixo.
                  final termoBusca = _buscaController.text
                      .toLowerCase()
                      .trim(); // pega o que o usuario digitou na busca, deixa tudo minusculo pra comparar sem erro de maiusculo/minusculo, e tira os espacos em branco desnecessarios com o trim()
                  final bateNoTitulo = nota.titulo.toLowerCase().contains(
                    termoBusca,
                  ); // verifica se o titulo da nota, transformado em minusculo, contem o texto da busca. se tiver, bate com o titulo
                  final bateNoConteudo = nota.conteudo.toLowerCase().contains(
                    termoBusca,
                  ); // mesma coisa, so que com o conteudo da nota. se tiver, bate no conteudo, bate com o conteudo
                  final bateNaBusca =
                      termoBusca.isEmpty ||
                      bateNoTitulo ||
                      bateNoConteudo; // a busca só bate se o termo de busca estiver vazio (ou seja, o usuario nao digitou nada, entao todas as notas passam) ou se bater no titulo ou se bater no conteudo.

                  // verifica a categoria clicada nas tags
                  final bateNaCategoria =
                      _filtroCategoria == 'Todas' ||
                      nota.categoria ==
                          _filtroCategoria; // a categoria só bate se o filtro selecionado for "todas" ou se a categoria da nota for igual ao filtro selecionado

                  return bateNaBusca &&
                      bateNaCategoria; // ou seja, a nota so passa pelo .where se for aprovada em ambos os testes
                }).toList();

                if (notasFiltradas.isEmpty) {
                  // se nao aparecer nenhuma nota, mostra uma mensagem avisando
                  return const Center(
                    child: Padding(
                ...
                return ListView.builder(
                  // constroi apenas os cards visiveis na tela do usuario, pra nao travar o celular com telas excessivas
                  shrinkWrap:
                      true, // encolhe essa lista que está dentro da listview principal do codigo, pra nao dar conflito, entao ocupa so o tamanho exato das notas que tao dentro dela.

                  physics:
                      const NeverScrollableScrollPhysics(), // desativa o scroll dessa listview interna que criamos, pro scroll ficar mais suave

                  itemCount: notasFiltradas
                      .length, // quantidade de itens que o listview vai criar, baseado na quantidade de notas filtradas que tem pra mostrar

                  itemBuilder: (context, index) {
                    // cria o loop pra rodar de nota em nota

                    final nota =
                        notasFiltradas[index]; // pega a nota atual do loop, baseado no index que o listview passa

                    String dataFormatada =
                        "${nota.dataHora.day.toString().padLeft(2, '0')}/${nota.dataHora.month.toString().padLeft(2, '0')}/${nota.dataHora.year} ${nota.dataHora.hour.toString().padLeft(2, '0')}:${nota.dataHora.minute.toString().padLeft(2, '0')}";
      ...
      child: Column(
        ...
        children: [
          ...
          TextField(
            ...
            style: const TextStyle(color: Colors.white),
            decoration: InputDecoration(
              // decora campos de texto em flutter
              hintText:
          ...
          TextField(
            ...
            maxLines:
                3, // nao limita as linhas de fato, ele eh mais pra limitar a altura visualmente
            style: const TextStyle(color: Colors.white),
            decoration: InputDecoration(
              hintText: 'Conteúdo...',
              hintStyle: const TextStyle(
            ...
            Row(
              ...
              children: [
                ...
                Container(
                  ...
                  child: Text(
                    categoria
                        .toUpperCase(), // faz o texto ficar todo em maiusculo
                    style: const TextStyle(
                      color: Colors.white,
                      fontSize: 10,
                      fontWeight: FontWeight.bold,
                ...
                Row(
                  children: [
                    ...
                    IconButton(
                      ...
                      onPressed: () {
                        List<String> partesData = dataHora
                            .split(' ')[0]
                            .split(
                              '/',
                            ); // pega a parte da data (oq vem antes do espaco) com split(' ') e depois separa dia, mes e ano com outro split
                        ...
                        DateTime dataDoCard = DateTime(
                          ano,
                          mes,
                          dia,
                        ); // retransforma pra DateTime p poder editar no calendario nativo

                        final notaPreenchida = Nota(
                          // cria o modelo nota preenchido com os dados atuais, p poder enviar p tela de editar
                          id: id,
                          titulo: titulo,
                          categoria: categoria,
                          conteudo: conteudo,
                    ...
                    IconButton(
                      ...
                      constraints:
                          const BoxConstraints(), // tira o padding padrao do IconButton, que eh meio grande,
                      onPressed: () => _confirmarDeletar(
                        id,
                      ), // quando clicado, o icone apaga a nota em questao com a funcao deletarNota
                    ),
                  ],
                ),
              ],
